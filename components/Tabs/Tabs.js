...
class TabLink {
  constructor(element) {
    ...
    this.data = this.element.getAttribute('data-tab');
    // Using the custom data attribute get the associated Item element
    this.itemElement = document.querySelectorAll('.tabs-item').forEach((elem)=>{
      // console.dir(elem)
       if(elem.dataset['tab'] === this.data ){
       return elem;
       }
    })
  ...
  select() {
    ...
    Array.from(links).forEach((elem)=>{
     console.log(elem.classList.remove('tabs-link-selected'))
    });
...
//pass in each link as a parameter
